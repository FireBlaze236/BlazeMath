...
namespace BlazeMath {
	...
	struct Vec2D {
		...
		inline float length_squared() { return x * x + y * y; }
		inline float length() { return std::sqrt(length_squared()); }
		...
		inline Vec2D normalized() { float l = 1.0f / length(); return Vec2D(x * l, y * l); }
	...
	struct Vec3D {
		...
		inline Vec3D normalized() { float l = 1.0f / length(); return Vec3D(x * l, y * l, z * l); }
	...
	struct Vec4D {
		...
		inline float length_squared() { return x * x + y * y + z * z + w * w; }
		inline float length() { return std::sqrt(length_squared()); }
		inline Vec4D normalized() { float l = 1.0f / length(); return Vec4D(x * l, y * l, z * l, w * l); }
